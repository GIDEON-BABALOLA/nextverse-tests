...
export const useRemoveANote = () => {
    const [error, setError] = useState(null)
    const [isLoading, setIsLoading] = useState(false)
    const [statusCode, setStatusCode] = useState(null)
    const [data, setData] = useState([])
    const removeANote = async (id) => {
        setIsLoading(true) //starting the request
        try{
            setError(null)
const response = await axiosConfig.delete(`/note/remove-note/${id}`,
    {
...
setError({message : "Your Request Has Timed Out", code : error.code})
    ...
    return {removeANote, isLoading, error, data, statusCode} 
}
