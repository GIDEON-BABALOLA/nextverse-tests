...
export const useCreateAReport = () => {
    const [error, setError] = useState(null)
    const [isLoading, setIsLoading] = useState(false)
    const [statusCode, setStatusCode] = useState(null)
    const [data, setData] = useState({})
    const createAReport = async (category, content, username) => {
        setIsLoading(true) //starting the request
        try{
            setError(null)
const response = await axiosConfig.post(`/report/create-report`,
    {
    category : category,
    content : content,
    username : username
...
setError({message : "Your Request Has Timed Out", code : error.code})
    ...
    return {createAReport, isLoading, error, data, statusCode} 
}
