...
export const useUnLikeAStory = () => {
    const [error, setError] = useState(null)
    const [isLoading, setIsLoading] = useState(true)
    const [statusCode, setStatusCode] = useState(null)
    const [data, setData] = useState([])
    const unlikeAStory = async (id) => {
        setIsLoading(true) //starting the request
        try{
            setError(null)
const response = await axiosConfig.patch(`/story/unlike-a-story/${id}`,
    {
...
setError({message : "Your Request Has Timed Out", code : error.code})
    ...
    return {unlikeAStory, isLoading, error, data, statusCode} 
}
