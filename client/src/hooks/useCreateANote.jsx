...
export const useCreateANote = () => {
    const [error, setError] = useState(null)
    const [isLoading, setIsLoading] = useState(false)
    const [statusCode, setStatusCode] = useState(null)
    const [data, setData] = useState([])
    const createANote = async (title, content) => {
        setIsLoading(true) //starting the request
        try{
            setError(null)
const response = await axiosConfig.post(`/note/create-note`,
    {
        title : title,
        content : content
    }, 
...
setError({message : "Your Request Has Timed Out", code : error.code})
    ...
    return {createANote, isLoading, error, data, statusCode} 
}
